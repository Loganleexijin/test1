...
export function CircularProgress({
  ...
  const elapsedHours = currentHours + currentMinutes / 60 + currentSeconds / 3600;
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="text-xs font-semibold text-muted-foreground font-mono mb-1">{Math.round(progress)}%</div>
        ...
        <div className="mt-3 text-center">
          <div className="text-sm font-bold text-warning">{getPhaseLabel(elapsedHours)}</div>
          <div className="text-[11px] text-muted-foreground mt-0.5">身体正在适应中</div>
        </div>
      </div>
    </div>
  );
